...
export async function renderFitness() {
  ...
  root.querySelectorAll('.rg-btn').forEach(b =>
    b.addEventListener('click', () => { view = b.dataset.view; renderFitness(); }));
  ...
  if (!resizeWired) {
    window.addEventListener('resize', () => {
      ...
      rzT = setTimeout(() => {
        ...
        if (currentRoot && page && page.classList.contains('active')) drawFitnessChart(currentRoot);
      }, 150);
    });
    resizeWired = true;
  }
...
function buildModel(runs) {
  ...
  return states.map((state, i) => {
    ...
    fitnessMomentum += ((state.fitness - prevFitness) - fitnessMomentum) * fitnessAlpha;
    fatigueMomentum += ((state.fatigue - prevFatigue) - fatigueMomentum) * fatigueAlpha;
...
function runSignal(today) {
  const raw = 50 + today.form * 1.4 + today.fitnessMomentum * 4 - today.fatigueMomentum * 7;
...
function drawFitnessChart(root) {
  ...
  const points = smoothChartPoints(chartPoints(RUNS, start, end), CHART_SMOOTH_SIGMA_DAYS * DAY);
  if (points.length < 2) {
    host.innerHTML = fitnessChart(smoothChartPoints(MODEL, CHART_SMOOTH_SIGMA_DAYS * DAY), Math.max(300, Math.round(host.clientWidth)));
    return;
  }
  host.innerHTML = fitnessChart(points, Math.max(300, Math.round(host.clientWidth)));
}
...
function chartPoints(runs, start, end) {
  ...
  times.push(end);
  return statesAtTimes(runs, [...new Set(times)].sort((a, b) => a - b));
}
...
function fitnessChart(points, W) {
  ...
  const x = t => padL + (W - padL - padR) * (t - points[0].t) / Math.max(1, points[points.length - 1].t - points[0].t);
  const y = v => H - padB - (H - padT - padB) * (v - lo) / Math.max(1, hi - lo);
  const path = key => 'M ' + points.map(p => `${x(p.t).toFixed(1)} ${y(p[key]).toFixed(1)}`).join(' L ');
  const zero = y(0);
